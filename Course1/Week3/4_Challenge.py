...
# when start is bigger than stop, and counts up from start to stop otherwise. 
...
# that are divisible by 2, up to and including the maximum that's 
# passed into the function. For example, even_numbers(6) returns “2 4 6”. 
...
# b)Failure to initialize variables
    #CORRECT Well done! The variable year needs to be initialized prior
    #  to being used in the while loop.
...
# d)votes(['yes', 'no', 'maybe'])
    # Correcto
    # Excellent! This function is looking for one argument, 
    # and the list of strings is just one argument. 
